...
public class WritePDF {
    ...
    public void writePN(int maphieu) {
        ...
        try {
            ...
            document.add(Chunk.NEWLINE);
            // Thêm table 4 cột vào file PDF
            PdfPTable table = new PdfPTable(4);
            table.setWidthPercentage(100);
            table.setWidths(new float[]{20f, 40f, 25f, 15f});
            PdfPCell cell;
            ...
            table.addCell(new PdfPCell(new Phrase("Khu vực kho", fontBold15)));
            table.addCell(new PdfPCell(new Phrase("Số lượng", fontBold15)));;
            for (int i = 0; i < 5; i++) {
                cell = new PdfPCell(new Phrase(""));
                table.addCell(cell);
            }
            for (ChiTietPhieuNhapDTO ctp : ChiTietPhieuNhapDAO.getInstance().selectAll(maphieu + "")) {
                SanPhamDTO sp = SanPhamDAO.getInstance().selectById(String.valueOf(ctp.getMaSanPham()));
                KhuVucKhoDTO kvk = KhuVucKhoDAO.getInstance().selectById(String.valueOf(ctp.getMaKho()));
                table.addCell(new PdfPCell(new Phrase("SP-" + sp.getMasp(), fontNormal10)));
                table.addCell(new PdfPCell(new Phrase(sp.getTensp(), fontNormal10)));
                table.addCell(new PdfPCell(new Phrase(kvk.getTenkhuvuc(), fontNormal10)));
            ...
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thành tiền: " + formatter.format(pn.getTongSP()) + "đ", fontBold15));
...
//            }
            ...
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thành tiền: " + formatter.format(px.getTongSP()) + "đ", fontBold15));
            paraTongThanhToan.setIndentationLeft(300);
