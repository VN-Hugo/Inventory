...
public class KhachHangDAO {
    ...
    public int insert(KhachHangDTO kh) {
        int result = 0;
        String sql = "INSERT INTO KhachHang (makh, hoten, sdt, diachi, ngaythamgia, trangthai) VALUES (?, ?, ?, ?, ?, 1)";
        try (Connection conn = ConnectionUtils.getMyConnection();
             PreparedStatement pst = conn.prepareStatement(sql)) {
    ...
    public int update(KhachHangDTO kh) {
        int result = 0;
        String sql = "UPDATE KhachHang SET hoten = ?, sdt = ?, diachi = ?, ngaythamgia = ? WHERE makh = ?";
        try (Connection conn = ConnectionUtils.getMyConnection();
             PreparedStatement pst = conn.prepareStatement(sql)) {
    ...
    public int getAutoIncrement() {
        int result = -1;
        String sql = "SELECT seq_khachhang_id.NEXTVAL FROM dual";  
        try (Connection conn = ConnectionUtils.getMyConnection();
             PreparedStatement pst = conn.prepareStatement(sql);
             ResultSet rs = pst.executeQuery()) {
            if (rs.next()) {
